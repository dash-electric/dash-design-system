...
`dashkit add` does that walk for you. One command, dependencies resolved, vars merged, files written into the paths your project already uses.
...
dashkit add button card sidebar

# 3. Find what's available
dashkit search "table"
dashkit list --type page
```

`dashkit init` is idempotent. Run it again to re-sync `components.json` after pulling new defaults.
...
```bash
dashkit add button                 # one atom
dashkit add data-table             # pulls table, input, button, icon-button…
dashkit add dashboard-shell        # full page template + every shell dep
dashkit add login-aurora register-aurora reset-password-key
```
...
```bash
dashkit info              # pretty table for humans
dashkit info --json       # single JSON object — for tooling / skills / CI
```
...
### `dashkit search <query>`

Full-text search across `name`, `title`, `description`, `categories`.

```bash
dashkit search "auth"              # all auth-related items
dashkit search "finance dashboard" # vertical-specific templates
dashkit search "settings" --type page
```

Use this before `dashkit add` to confirm the exact name. The registry has 178 items — guessing rarely works.
...
```bash
dashkit doctor                      # pretty output
dashkit doctor --json               # machine output (CI / tooling)
dashkit doctor --no-network         # offline diagnostic (skips registry + token probes)
dashkit doctor --registry http://localhost:3000   # override registry for the check
```
...
```bash
dashkit mcp init                    # auto-detect installed editors
dashkit mcp init --claude-code      # Claude Code only
dashkit mcp init --cursor           # Cursor only
dashkit mcp init --both             # both editors in one shot
dashkit mcp init --check-only       # detect + report, no writes
```
...
| Command | Description |
| --- | --- |
| `dashkit init` | Initialize `components.json`, base tokens, AI rules |
| `dashkit add <name...>` | Install items, resolve `registryDependencies`, merge CSS vars |
| `dashkit info` | Print project snapshot (framework, aliases, installed items) — pretty or `--json` |
| `dashkit search <query>` | Full-text search the registry |
| `dashkit list` | List all items (filter with `--type ui\|block\|page\|hook`) |
| `dashkit diff <name>` | Compare local copy of an item against latest registry version |
| `dashkit build` | Build registry JSON from source (only inside the `dash-ds` repo) |
| `dashkit mcp init` | Wire `@dash/mcp-server` into Claude Code (`~/.claude/mcp-config.json`) and/or Cursor (`~/.cursor/mcp.json`). Auto-detects installed editors. Flags: `--claude-code`, `--cursor`, `--both`, `--check-only` |
| `dashkit doctor` | End-to-end health check (registry reachable, token valid, MCP wired per editor, framework, Node, package manager, workspace root). Flags: `--json`, `--registry <url>`, `--no-network` |
...
# Or per-command
dashkit add button --token dash_pat_xxx
```
...
You've edited the installed file and the registry version drifted. Run `dashkit diff <name>` to see what changed; merge by hand if your local edits matter, otherwise pick `[o]verwrite`.

**CSS vars duplicated after `dashkit add`**
...
dashkit init --yes --registry-url http://localhost:3000
dashkit add button card sidebar --yes
dashkit list --registry-url http://localhost:3000
```
...
> If you see `Warning: missing peer tailwindcss>=3.0.0` after `dashkit init`, install Tailwind in the consumer project: `pnpm add -D tailwindcss postcss autoprefixer && pnpm dlx tailwindcss init`. The CLI does not auto-install Tailwind to avoid overwriting your existing config.
...
- `dashkit diff` is line-by-line, not a proper unified diff.
...
- [`dash-mcp`](../dash-mcp/README.md) — the MCP server for AI agents (wired via `dashkit mcp init`).
