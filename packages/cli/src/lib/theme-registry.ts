 ...
 * consumer-side `dash add --theme <x>` keeps working before all manifests
 ...
 * theme), so `dash add --theme <x>` always produces something on disk.
 */
export function readThemeColorsCss(
  ...
  return [
    ...
    `/* Generated by \`dash add --theme ${name}\` — Phase C scaffold. */`,
    `:root {`,
