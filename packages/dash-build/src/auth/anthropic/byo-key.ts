// Bring-your-own API key fallback for users not on Pro/Team or when OAuth fails.
// Uses the same encryption + storage primitives as the OAuth token store.
...
const KEY_FILENAME = "anthropic-byo-key.json"
const SCRYPT_SALT = "dash-build-anthropic-byo-v1"
...
export class BYOKeyStore {
  ...
  async save(apiKey: string): Promise<void> {
    if (!apiKey || !apiKey.startsWith("sk-")) {
      throw new Error('Invalid Anthropic API key (expected prefix "sk-").')
    }
