...
async function seedFoundation(root: string, opts: { cardinal?: string; voice?: string; manifest?: object; layered?: string } = {}) {
...
describe("loadDesignContext", () => {
  it("loads all four foundation files when present", async () => {
    ...
    expect(ctx.loadedSources.length).toBe(4)
  ...
  it("loads what exists when manifest.json is missing", async () => {
    ...
    expect(ctx.manifest).toBeNull()
    expect(ctx.loadedSources.length).toBe(2)
